...
void CTim::SavePng(LPCSTR filename)
{
	lodepng::State state;

	for (int i = 0, si = pal_w * pal_h; i < si; i++)
	{
		u32 color = ToRGB888_full(clut[i]);
	...
	switch (bpp)
	{
	case 4: w = pix_w * 4; break;
	case 8: w = pix_w * 2; break;
	}
	...
	lodepng::save_file(out, filename);
}
...
void CBitmap::SavePng(LPCSTR name, bool no_alpha)
{
	...
	for (size_t y = 0; y < this->h; y++)
	{
		for (size_t x = 0; x < this->w; x++)
		{
			...
			if (no_alpha)
				*out++ = ((p & 0xff) << 16) | ((p & 0xff0000) >> 16) | (p & 0xff00) | 0xff000000;
			else
